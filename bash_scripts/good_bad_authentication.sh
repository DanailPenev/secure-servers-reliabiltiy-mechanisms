...
do
        ...
        do
                for i in {1..3}
                do
                        wrk -t32 -c$c -d5m -s../wrk_scripts/csv_good_authentication.lua $hostname:8080/$mode/login
                done
        done
done
...
do
	...
	do
		for i in {1..3}
		do
			wrk -t32 -c$c -d5m -s../wrk_scripts/csv_bad_authentication.lua $hostname:8080/$mode/login
		done
	done
done
exit 0
